...
onMounted(async () => {
  const res = await productLabel.getRow(props.id);
  if (res) {
...
async function submitData() {
  ...
  if (
    !productLabelForm.value.title ||
    productLabelForm.value.title.length === 0
  ) {
  ...
  try {
    if (productLabelForm.value.my_id.length > 0) {
      ...
      await productLabel.update(productLabelForm.value.my_id, row);
      ElMessage.success("更新成功");
    } else {
      const row: ProductLabelRow = toRaw(productLabelForm.value);
      row.my_id = FooseTools.createUuid();
      const newRow = await productLabel.create(row);
      if (newRow) {
    ...
    productLabel.errorInfo = e instanceof Error ? e.message : String(e);
  } finally {
    productLabel.loading = false;
  }
}

//父组件中的方法
const handleEditLabelClose = inject<(reload: boolean) => void>(
  "handle-edit-label-close",
  () => {}
);
</script>
<template>
  <div>
    <div v-if="productLabel.errorInfo">{{ productLabel.errorInfo }}</div>
    <div>
      <el-form
        :model="productLabelForm"
        label-width="120"
        style="max-width: 100%"
      >
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="产品标签名称" prop="title">
              <el-input
                v-model="productLabelForm.title"
                maxlength="50"
                minlength="2"
                placeholder="请输入产品标签名称"
              />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="产品ID">
              <el-input
                v-model="productLabelForm.product_id"
                type="number"
                min="1"
                max="999999"
                placeholder="请输入产品ID"
