...
import { DEFAULT_OBJECT_NAME, importFooseClient } from "./demo/foose_base";
...
> = K extends `foose${infer Rest}` ? `${Prefix}${Rest}` : K;

export type PrefixedComposable<T extends FooseRow, Prefix extends string> = {
  [
    K in keyof FooseComposable<T> as ReplaceFoose<Extract<K, string>, Prefix>
  ]: FooseComposable<T>[K];
};
...
>(
  ...
  return function useFoose(): PrefixedComposable<T, Prefix> {
    ...
    const result = rename
      ? renameKeys(raw as unknown as Record<string, unknown>, prefix)
      : raw;
