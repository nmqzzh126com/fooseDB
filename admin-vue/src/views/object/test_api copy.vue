        ...
        <div>
          <div class="p-2">
            <div v-if="false" class="text-right">
              <el-button type="primary" size="small" plain :loading="testStatus === 'loading'"
            ...
            <el-text type="primary">
              {{ pageUrl }}/{{ resolvedApiName }}
            </el-text>
            <div>
              <div v-if="resolvedDbUrl">
                <el-text type="info"> 数据库连接字符: </el-text>
                <el-text type="warning">db_url: {{ resolvedDbUrl }}</el-text>
              </div>
              <div v-if="resolvedDbPath">
                <el-text type="info"> 数据库路径: </el-text>
                <el-text type="info">db_path: {{ resolvedDbPath }}</el-text>
              </div>
              <div>
                      ...
                      <el-collapse-item name="advanced">
                        ...
                        <div class="flex flex-col gap-2 pt-1">
                          <div class="flex gap-2 items-center">
                            ...
                            <el-input v-model="customBaseUrl" size="small"
                              placeholder="/api 或 http://192.168.1.100:8858/api" clearable />
                            <el-text size="small" type="info">默认同源 /api</el-text>
                          </div>
                          <div class="flex gap-2 items-center">
                            <el-text size="small" class="w-28 shrink-0">Bearer Token</el-text>
                            <el-input v-model="customToken" size="small" placeholder="可手动粘贴外部 token（通常由上方登录自动填充）"
          ...
          <el-splitter-panel size="50%">
            <div class="p-2">
              ...
              <el-tabs v-model="activeName" tab-position="left" type="border-card">
                <el-tab-pane label="列表" name="get-page-list">列表</el-tab-pane>
              ...
              <el-card shadow="never" class="flex flex-col gap-3">
                ...
                <div v-if="method === 'get-list'" class="flex flex-col gap-2">
                  <div class="flex gap-2 flex-wrap items-center">
                    <el-input-number v-model="paramPage" :min="1" :max="9999" label="page" size="small" />
                    <el-input-number v-model="paramPageSize" :min="1" :max="5000" label="pageSize" size="small" />
                    <el-input v-model="paramOrderBy" placeholder="orderBy，如 created_at 或 created_at,-id" size="small"
                      class="w-56" />
                  ...
                  <div class="flex gap-2 flex-wrap items-center">
                    <el-input v-model="paramFields" placeholder="fields 列裁剪，逗号分隔：id,name,title" size="small"
                      class="w-72" />
                  ...
                  <div class="flex flex-col gap-1">
                    ...
                    <div v-for="(f, i) in filters" :key="i" class="flex gap-2 items-center">
                      <el-input v-model="f.key" placeholder="key 如 status 或 id__gte" size="small" class="w-48"
                        clearable />
                      ...
                      <el-button v-if="i === filters.length - 1" size="small" type="primary" :icon="Plus" circle
                        @click="addFilter" />
                    </div>
                  </div>
                </div>
