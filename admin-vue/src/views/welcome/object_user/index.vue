...
import { useTabObjectUser } from "@/utils/hooks";
const { initToTab, getParameter } = useTabObjectUser();
initToTab();
...
const AddEditUser = defineAsyncComponent(async () => {
  return import("@/views/welcome/object_user/edit.vue");
});
...
const userList = ref<BusinessUser[]>([]);
const multipleTableRef = ref<TableInstance>()
const multipleSelection = ref<BusinessUser[]>([])//所有选择的表
const selectedUserIds = computed<number[]>(() => multipleSelection.value.map(item => item.id))
// 添加或编辑用户
const addEditDialogVisible = ref(false);// 添加或编辑用户弹窗是否显示
const userId = ref(0);// 当前用户ID
const dialogTitle = computed(() => userId.value > 0 ? "编辑用户" : "添加用户");// // 弹窗标题
// 批量设置用户
...
const loadPageList = async () => {
  ...
  try {
    const res = await listUsersByObject(object_id.value, {
      page: defautPageIndex.value,
      pageSize: defaultPageSize.value,
      username: queryUserName.value?.trim() || undefined,
      nickname: queryNickname.value?.trim() || undefined
    });
...
/**哪些不允许选择 */
const selectable = (row: BusinessUser) => {
  return true;// 允许选择所有表   
}
// 选择改变时,更新选择列表,确定所有选择的表IDs
const handleSelectionChange = (val: BusinessUser[]) => {
...
async function handleRemove(rowId: number, username: string, nickname: string) {
  ...
  const { ok } = await deleteUser(rowId);
  if (!ok) {
...
async function handleSetFlag(rowId: number, flag: number) {
  ...
  const res: BusinessUser = await updateUser(rowId, { flag: flag as 0 | 1 });
  if (!res || !res.id) {
...
async function handleBatchRemove() {
  ...
  const confirmResult = await confirm(`确认批量移除 ${selectedUserIds.value.length} 个用户吗？`);
  if (!confirmResult) {
    return;
  }
  let errorCount = 0;
...
async function handleBatchSetConfirm() {
  ...
  const updateRow = {};
  batchRow.value.new_password = clearAllSpace(batchRow.value.new_password || "");
  //batchRow.value.confirm_password = clearAllSpace(batchRow.value.confirm_password || "");
  if (batchRow.value.new_password !== "") {
    const passwordValidateResult = validatePassword(batchRow.value.new_password);
    if (!passwordValidateResult.valid) {
      ElMessage.error(passwordValidateResult.message);
      return;
    }
  ...
  const confirmResult = await confirm(`确认批量设置 ${selectedUserIds.value.length} 个用户信息吗？`);
  if (!confirmResult) {
    return;
  }
  //console.log("batch rows:", updateRow);
  const { ok } = await batchUpdateUsers({ userIds: selectedUserIds.value, ...updateRow });
  if (!ok) {
    ElMessage.error("设置失败");
    return;
  }
  ElMessage.success(`设置成功 ${selectedUserIds.value.length} 个用户`);
  debounceLoadList();
}
/** 生成随机密码 */
async function createPassword() {
  batchRow.value.new_password = generatePassword();
  //batchRow.value.confirm_password = batchRow.value.new_password;
}
</script>
<template>
  <div>
    <div class="flex_row mb-3">
      <div class="left">
        <el-row :gutter="8">
          ...
          <el-col :span="8">
            <el-input v-model="queryUserName" placeholder="用户名" clearable style="width: 100%"
              @keyup.enter="debounceLoadList" @clear="debounceLoadList" />
          </el-col>
          <el-col :span="8">
            <el-input v-model="queryNickname" placeholder="用户昵称" clearable style="width: 100%"
              @keyup.enter="debounceLoadList" @clear="debounceLoadList" />
          </el-col>
        </el-row>
      </div>
      <div class="right">
        ...
        <el-button class="ml-5!" :disabled="selectedUserIds.length < 1" plain :icon="Setting" type="warning"
          @click="batchDialogVisible = true">批量设置</el-button>
        <el-button :icon="CirclePlus" plain type="primary" @click="handleAdd">添加用户</el-button>
      </div>
    </div>
    <div class="m-2">
      <el-table ref="multipleTableRef" :data="userList" row-key="id" stripe border style="width: 100%"
        ...
        <el-table-column prop="flag" label="用户状态" width="100" align="center">
          <template #default="scope">
        ...
        <el-table-column fixed="right" label="操作" width="200" align="center">
          <template #default="scope">
            ...
            <el-button title="移除用户" type="danger" class="my-button" plain :icon="Delete" size="small"
              @click="handleRemove(scope.row.id, scope.row.username, scope.row.nickname)" />
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 批量设置 -->
    <el-dialog v-model="batchDialogVisible" title="批量设置" width="500px" align-center>
      <div class="p-2">
        <el-form :model="batchRow" label-width="auto" width="100%">
          ...
          <el-form-item label="重置密码">
            <el-input v-model="batchRow.new_password" clearable placeholder="请输入新密码" />
          </el-form-item>
          <el-form-item label="自动生成密码">
            <el-button type="success" plain size="small" @click="createPassword">生成密码</el-button>
          </el-form-item>
        </el-form>
      ...
      <template #footer>
        <el-row>
          <el-col :span="12" class="text-left">
            <el-text type="danger" class="ml-5!">批量操作 {{ selectedUserIds.length }} 个用户</el-text>
          </el-col>
          <el-col :span="12">
            <div class="dialog-footer">
              <el-button @click="batchDialogVisible = false">取消</el-button>
              <el-button :disabled="selectedUserIds.length < 1" type="primary" @click="handleBatchSetConfirm">
                确认设置
              </el-button>
            </div>
          </el-col>
    ...
    <el-dialog v-model="addEditDialogVisible" :title="dialogTitle" width="70vw" align-center
      ...
      <add-edit-user :object-id="object_id" :user-id="userId" />
    </el-dialog>
  </div>
</template>
