...
<template>
  <div>
    <div class="flex_row mb-3">
      ...
      <div class="right">
        <el-button type="primary" :loading="productDbLoading" :icon="Search" @click="debounceLoadList">查询</el-button>
        ...
        <el-button :icon="CirclePlus" type="warning" @click="handleBatchAdd">批量添加</el-button>
        <el-text v-if="productDbError" class="mx-1" type="danger">
          {{ productDbError }}
        </el-text>
      </div>
    </div>
    <el-table :data="productDbDataList" row-key="id" stripe border style="width: 100%">
      ...
      <el-table-column prop="product_count" label="数量" width="100" sortable align="left">
        <template #default="scope">
          {{ scope.row.product_count || 0 }}
        </template>
      </el-table-column>
      <el-table-column prop="product_desc" label="产品描述" width="200" align="left">
        <template #default="scope">
          <el-text class="mx-1">
      ...
      <el-table-column label="产品标签" width="200" align="left">
        <template #default="scope">
          <el-tag v-for="(label, index) in scope.row.pl" :key="index" type="info" size="small">
            {{ label.title || "--" }}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column fixed="right" label="操作" width="200" align="center">
        <template #default="scope">
          ...
          <el-button type="danger" :icon="Delete" @click="handleRemove(scope.row.id)">
            删除
          </el-button>
        </template>
      </el-table-column>
    ...
    <div class="mt-3 pagination-container">
      <el-pagination v-model:current-page="productDbPage" v-model:page-size="productDbPageSize" size="default"
        :page-sizes="[5, 10, 20, 50, 100]" layout="total, sizes, prev, pager, next, jumper" :total="productDbTotal"
        @size-change="handleSizeChange" @current-change="handleCurrentChange" />
    </div>
    <!-- 编辑弹窗 -->
    <el-dialog v-model="dialogEditVisible" :title="currentProductId > 0 ? '编辑产品' : '添加产品'" align-center width="70vw"
