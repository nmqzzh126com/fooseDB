...
function initRouter() {
  const fallbackToStatic = () => {
    // 后端没有 /get-async-routes 时，使用静态路由兜底（constantMenus）
    handleAsyncRoutes([]);
    return router;
  };
        ...
        getAsyncRoutes()
          .then(({ code, data }) => {
            if (code === 0) {
              ...
              resolve(fallbackToStatic());
            }
          })
          .catch(() => resolve(fallbackToStatic()));
      });
    }
  } else {
    return new Promise(resolve => {
...
function getTopMenu(tag = false): menuType {
  ...
  // 防御：wholeMenus 为空时（后端路由加载失败）返回静态首页兜底
  if (!menus || menus.length === 0 || !menus[0]?.children?.length) {
    const fallback = { path: "/welcome", name: "Home" };
    if (tag) useMultiTagsStoreHook().handleTags("push", fallback);
    return fallback as menuType;
  }
