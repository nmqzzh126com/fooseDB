...
export interface DataInfo<T> {
  ...
  /** admin-panel 识别标识（admin 面板 JWT 带此值） */
  scope?: string;
...
export function setToken(data: DataInfo<Date | string>) {
  ...
  // data.expires 现在是 ISO 字符串（api/user.ts 从 unix 秒转来）
...
/**
 * 判断当前登录用户是否为 admin-panel（管理端面板专属）。
 * 识别依据：登录时后端 JWT payload 带 scope="admin-panel"，
 * api/user.ts 解码后存入 localStorage user-info。
 */
export function isAdminPanel(): boolean {
  const user = storageLocal().getItem<DataInfo<number>>(userKey);
  return user?.scope === "admin-panel";
}
